...
\begin{document}
	...
	\begin{columns}
		...
		\block{Motivation}{
			...
			The concept of different finiteness definitions that are nonequivalent without the axiom of choice was explored greatly (see \cite{Levy:0,Tarski:0,Truss:0}). The different definitions, apart from being interesting in it's own, gave a nice list of pathological objects that may exist in $\sf ZF$ as well as discovering more structures that may naturally arise in certain models of $\sf ZF$ (see \cite{Sageev:0}).
		...
		\block{Structure of the Definitions}{
		 	Originally when Dedekind came up with the definition of what we call today Dedekind infinite set he noted that his definition was the first definition about finiteness that did not reference the natural numbers $\omega$. The general idea of the rest of the definitions for finiteness remain in a similar vain, looking at the structure of $X$ (or $\mathcal P(X)$, $X^X$) we find a property that characterize finiteness/infiniteness when we assume the axiom of choice, and then we can analyze that definition in $\sf ZF$.
		}
		
		\column{0.36}
		\block{Construction}{
			Given a set $X$ an operator from $\mathcal P(X)$ is called \textit{$\star$-complement} if it is a non-complement involution that whose image is disjoint from origin, that is: $^*$ is called $\star$-complement whenever
			\begin{itemize}
			...
			On the other hand, if $X$ is Dedekind infinite then there exists $\star$-complement operator for $X$. We may assume $\mathbb Z\subseteq X$, now if $a\subseteq X$ is of the form $\{q\in \mathbb Z\mid q>z\}$ for some $z\in \mathbb Z$, then let $a^*=\{q\in\mathbb Z\mid q<z\}$, for $a=\{q\in\mathbb Z\mid q<z\}$ let $a^*=\{q\in \mathbb Z\mid q>z\}$, otherwise let $a^*=a^\complement$.
		...
		\block{{\fontsize{60}{50}\selectfont$\star$-}Finite}{
			It turns out that a cardinality $\frak n$ have $\star$-complement operator if and only if it is infinite in the sense of $(4)$.
			...
			\end{tikzfigure}
			To see that we shall use the theorem of Kuratowski (see \cite{Tarski:0}), a cardinality is finite in the sense of $(4)$ if and only if $\omega$ does not surject onto it. That in turn is equivalent to having $\mathcal U\subseteq \mathcal P(X)$ such that $(\mathcal U,\subset)\cong(\mathbb Z,<)$. To construct $\star$-complement operator from being finite in the sense of $(5)$ we can replicate our construction from the previous part. 
		...
		\block{Remarks}{
			\begin{enumerate}
				\item A similar characteristic can be found to strongly $\star$-complement operator by finding partition of the infinite coinfinite subsets of $X$ into copies of $\mathbb Z$.
